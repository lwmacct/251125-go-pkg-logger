...
// levelColors 定义日志级别对应的 ANSI 颜色代码。
...
// keyColors 定义日志字段对应的 ANSI 颜色代码。
// 特定字段有专用颜色，未匹配的字段使用 "other" 对应的青色。
...
// coloredHandler 实现 [slog.Handler] 接口，支持 ANSI 彩色输出和字段排序。
//
// 该 handler 适用于终端输出，能自动平铺 JSON 和嵌套结构，
// 并按照 PriorityKeys/TrailingKeys 配置对字段排序。
// 使用 [NewColoredHandler] 创建实例。
...
// Handle 实现 [slog.Handler] 接口，格式化并输出一条日志记录。
//
